...
class HTMLFormatter:
    ...
    def format(self, transcript: Transcript) -> str:
        ...
        html.append(f"<li><strong>Customer Messages:</strong> {sum(1 for m in transcript.messages if m.role == 'user')}</li>")
        html.append(f"<li><strong>Agent Messages:</strong> {sum(1 for m in transcript.messages if m.role == 'assistant')}</li>")
        html.append(f"<li><strong>Interruptions:</strong> {sum(1 for m in transcript.messages if m.interrupted)}</li>")
        html.append("</ul>")
        html.append("</div>")
