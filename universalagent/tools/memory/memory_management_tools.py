...
class MemoryManagementTool:
    def __init__(self, memory_config: MemoryConfig, memory_manager: AsyncMemoryClient, user_id: str, agent_id: str):
    ...
    async def save_memory_from_messages(self, messages: List[ChatItem]):
        ...
        await self.memory_manager.add(messages_to_save, user_id=self.user_id, agent_id=self.agent_id)
    ...
    def get_memory_management_tools(self) -> List[ToolHolder]:
        ...
        return [
            ToolHolder(get_memory),
            ToolHolder(store_important_info)
        ]
